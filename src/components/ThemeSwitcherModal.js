...
export default function ThemeSwitcherModal({ visible, onClose }) {
  ...
  // 'visible' controla se está visível (anima entrada/saída).
  ...
  useEffect(() => {
    if (visible) {
      // Monta primeiro, depois anima entrada
  ...
  useEffect(() => {
    ...
    if (visible) {
      ...
      // Saída — SÓ DEPOIS da animação é que desmonta
      Animated.parallel([
...
const s = StyleSheet.create({
  ...
  previewBar: {
    height: 8, borderRadius: 4, width: '100%', marginBottom: 4,
  },
  previewCard: {
